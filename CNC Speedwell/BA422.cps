...
properties = {
  ...
  toolLengthCheckLowerPercent: {
    title      : "Tool Length Check Lower %",
    description: "Lower DP3 limit as a percentage of the programmed tool length.",
    group      : "preferences",
    type       : "number",
    value      : 75,
    scope      : "post"
  },
  toolLengthCheckUpperPercent: {
    title      : "Tool Length Check Upper %",
    description: "Upper DP3 limit as a percentage of the programmed tool length.",
    group      : "preferences",
    type       : "number",
    value      : 110,
    scope      : "post"
...
var toolLifeFormat = createFormat({decimals:0, minDigitsLeft:2});
...
var toolDataFormat = createFormat({decimals:(unit == MM ? 3 : 4)});
...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      {id:COOLANT_AIR},
      {id:COOLANT_AIR_THROUGH_TOOL,on:88},
      {id:COOLANT_SUCTION},
      {id:COOLANT_FLOOD_MIST},
      {id:COOLANT_FLOOD_THROUGH_TOOL, on:88},
    ...
    for (var i = 0; i < outputFormats.length; i++) {
      if (machineConfiguration.isMachineCoordinate(i)) {
        if (machineConfiguration.getAxisByCoordinate(i).isCyclic() || isSameDirection(machineConfiguration.getAxisByCoordinate(i).getAxis(), machineConfiguration.getSpindleAxis())) {
          ...
          // Siemens C=DC() uses shortest-path rotary positioning
          if (revision >= 50338) {
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    writeBlock("L6");
    if (isFirstSection()) {
      writeFirstToolpathSetupBlock();
    }
    writeToolLengthAndRadiusCheck(tool);
    if (isFirstSection()) {
      writeBlock(mFormat.format(1));
    }
    writeToolCheckCoolant(tool);
    dOutput.reset();
...
function writeToolLengthAndRadiusCheck(tool) {
  var toolLength = getBodyLength(tool);
  var lowerPercent = getProperty("toolLengthCheckLowerPercent");
  var upperPercent = getProperty("toolLengthCheckUpperPercent");
  if (upperPercent < lowerPercent) {
    var tempPercent = lowerPercent;
    lowerPercent = upperPercent;
    upperPercent = tempPercent;
  }
  var lowerLength = toolLength * (lowerPercent / 100.0);
  var upperLength = toolLength * (upperPercent / 100.0);
  var toolRadius = tool.diameter * 0.5;
  writeBlock("$TC_DP6[$P_TOOLNO,1]=" + toolDataFormat.format(toolRadius));
  writeBlock("IF ($TC_DP3[$P_TOOLNO,1]<" + toolDataFormat.format(lowerLength) + ")OR($TC_DP3[$P_TOOLNO,1]>" + toolDataFormat.format(upperLength) + ") GOTOF _END0");
}

function writeToolCheckCoolant(tool) {
  if (!tool || (tool.type == TOOL_PROBE)) {
    return;
  }
  var coolantCodes = getCoolantCodes(tool.coolant);
  if (Array.isArray(coolantCodes) && (coolantCodes.length > 0)) {
    writeBlock("WHEN TRUE DO " + coolantCodes.join(getWordSeparator()));
    forceCoolant = false;
  }
}

function onSectionEnd() {
  var toolSelector = getProperty("toolAsName") ? "description" : "number";
  var isToolChangeAtEnd = !isLastSection() && isToolChangeNeeded(getNextSection(), toolSelector);
  ...
  if (!isLastSection()) {
    ...
    if (tool.breakControl && isToolChangeAtEnd) {
      onCommand(COMMAND_BREAK_CONTROL);
    }
  }

  if (isToolChangeAtEnd || isLastSection()) {
    writeln("");
    writeBlock("G0 Z100 M56 M9 M72");
    currentCoolantMode = COOLANT_OFF;
    forceCoolant = false;
    var toolLifeRegister = "R7" + toolLifeFormat.format(tool.number);
    writeBlock(toolLifeRegister + "=" + toolLifeRegister + "+1");
  }
...
function alarms() {
  writeln("");
  writeToolBlock("T" + toolFormat.format(0));
  writeBlock("L6");
  writeBlock(gFormat.format(0), gFormat.format(90) + " AX[_AB]=DC(0)");
  writeBlock("R901=R901+2");
  writeBlock(mFormat.format(83));
  writeln("");
  writeBlock("$AC_TIMER[1]=-1");
  writeBlock("STOPRE");
  writeBlock("R0=($AC_TIMER[1])/60");
  writeBlock("SETPIECE(2)");
  writeBlock("RELEASE(A,B)");
  writeBlock("M30");
  writeBlock("_END0:");
  writeBlock("SETAL(66000)");
  writeBlock("M30");
  writeBlock("_END1:");
  writeBlock("SETAL(66001)");
  writeBlock("M30");
  writeBlock("_END2:");
  writeBlock("SETAL(66002)");
  writeBlock("M30");
  writeBlock("_END3:");
  writeBlock("SETAL(66003)");
  writeBlock("M30");
  writeBlock("_END4:");
  writeBlock("SETAL(66004)");
  writeBlock("M30");
  writeBlock("_END5:");
  writeBlock("SETAL(66005)");
  writeBlock("M30");
  writeBlock("_END6:");
  writeBlock("SETAL(66006)");
  writeBlock("M30");
  writeBlock("_END7:");
  writeBlock("SETAL(66007)");
  writeBlock("M30");
  writeBlock("_END8:");
  writeBlock("SETAL(66008)");
  writeBlock("M30");
  writeBlock("_END9:");
  writeBlock("SETAL(66009)");
  writeBlock("M30");
}
function onClose() {
...
properties.writeSetupUIFR = {
  title      : "Write setup UIFR lines",
  description: "Output one $P_UIFR[n]=CTRANS() line for each used setup/work offset.",
  group      : "formats",
  type       : "boolean",
  value      : true,
  scope      : "post"
...
function getUsedSetupSections() {
  var usedSetups = [];
  var usedSetupKeys = {};
  for (var i = 0; i < getNumberOfSections(); ++i) {
    var section = getSection(i);
    var setupKey = section.wcs ? section.wcs : ((section.workOffset > 0) ? ("W" + section.workOffset) : "");
    if (setupKey && !usedSetupKeys[setupKey]) {
      usedSetupKeys[setupKey] = true;
      usedSetups.push(section);
    }
  }
  return usedSetups;
}

function writeSetupUIFRDefinitions() {
  if (!getProperty("writeSetupUIFR")) {
    return;
  }
  var usedSetups = getUsedSetupSections();
  if (usedSetups.length == 0) {
    return;
  }

  writeComment("---------------------------------------------------");
  for (var i = 0; i < usedSetups.length; ++i) {
    var section = usedSetups[i];
    if (section.wcs) {
      writeComment("(" + section.wcs + " setup)");
    }
    writeBlock("$P_UIFR[" + integerFormat.format(i + 1) + "]=CTRANS()");
    writeComment("---------------------------------------------------");
  }
}

function writeFirstToolpathSetupBlock() {
  writeBlock("R99=$P_SEARCH");
  writeBlock("L20");
  writeBlock("L21(_AB,_UV,_U2V2)");
  writeBlock("L23(_ATISCH)");
  writeBlock("CHANDATA(1)");
  writeBlock("$P_UIFR[0]=CTRANS()");
  writeComment("---------------------------------------------------");
  writeBlock("IF _ATISCH <>1 GOTOF _END1");
  writeComment("---------------------------------------------------");
  writeSetupUIFRDefinitions();
  writeBlock("$P_UBFR=CTRANS()");
  writeComment("---------------------------------------------------");
  writeBlock("IF $AA_IM[Y] <430 GOTOF _END2");
  writeBlock("IF $AA_IM[Z] <345 GOTOF _END2");
  writeComment("---------------------------------------------------");
}
// <<<<< INCLUDED FROM include_files/writeProgramHeader.cpi
