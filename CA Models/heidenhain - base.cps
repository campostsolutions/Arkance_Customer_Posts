...
function positionABC(abc, force) {
  ...
  if (a || b || c) {
    ...
    machineSimulation({a:abc.x, b:abc.y, c:abc.z, coordinates:MACHINE, rotaryMode:SHORTEST}); // M94 uses shortest path
  }
}
// <<<<< INCLUDED FROM include_files/positionABC_heidenhain.cpi
...
function writeWCS() {
  if (currentSection.workOffset != currentWorkOffset) {
    ...
    if (revision >= 50338 && getCurrentSectionId() > 0 && currentSection.workOffset != getPreviousSection().workOffset) {
...
function writeProbeCycle(cycle, x, y, z) {
  ...
  case "probing-y":
    ...
    break;
  case "probing-z":
    Q261 = formatQword("Q261", xyzFormat.format(z - cycle.depth), "MEASURING HEIGHT");
  ...
  case "probing-y-channel-with-island":
    ...
    if (probeGeometry) {
      if (validateProbeResult(probeGeometry, QLvalue)) {
        writeBlock("FN 0: Q157=" + xyzFormat.format(QLvalue));
      }
...
function validateProbeResult(probeGeometry, QLvalue) {
  return probeGeometry && QLvalue != undefined && (cycle.outOfPositionAction && cycle.outOfPositionAction == "stop-message");
}
