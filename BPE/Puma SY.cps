/**
  ...
  $Revision: 44224 c2224c368e9f5db45e920540f4162bae265f1d67 $
  $Date: 2026-04-27 09:23:16 $
...
if (typeof revision == "number" && typeof supportedFeatures != "undefined") {
  supportedFeatures |= revision >= 50279 ? FEATURE_TOOL_CALL_CYCLE : 0;
}
...
properties = {
  ...
  cleanSubSpindle: {
    title      : "Clean sub-spindle",
    description: "Enable to output a cleaning routine for the sub-spindle before part transfer.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function getWorkPlaneMachineABC(section, workPlane) {
  ...
  if (tcp) {
    ...
    if (true) {
      var tempABC = (Math.abs(bFormat.getResultingValue(abc.y)) == 90) ?
...
function writeToolCall(_tool) {
  ...
  var compensationOffset = _tool.isTurningTool() ? _tool.compensationOffset : _tool.lengthOffset;
  if (offsetFactor == undefined) {
    offsetFactor = getToolOffsetFactor(tool);
  }
  var maxCompensationOffset = useExtendedOffsets ? 999 : 99;
  if (compensationOffset > maxCompensationOffset) {
    error(subst(localize("Compensation offset '%1' exceeds maximum value of '%2'."), compensationOffset, maxCompensationOffset));
  }
  if (_tool.number > getProperty("maxTool")) {
    warning(subst(localize("Tool number '%1' exceeds maximum value of '%2' defined by property '%3'."),
      _tool.number, getProperty("maxTool"), properties.maxTool.title)
    );
  }
  gMotionModal.reset();
...
function onCycle() {
  if (cycleType == "tool-call") {
    var insertToolCall = isToolChangeNeeded("number", "compensationOffset", "diameterOffset", "lengthOffset");
    if (machineState.spindlesAreAttached) {
    ...
    if (insertToolCall || cycle.toolCallPositionWorkCS || cycle.toolCallPositionMachineCS) {
      ...
      if (cycle.toolCallPositionWorkCS || cycle.toolCallPositionMachineCS) {
        ...
        if (cycle.useMachineFrame) {
          writeBlock(gMotionModal.format(0), gFormat.format(53), zOutput.format(cycle.toolCallPositionMachineCS));
        } else {
          writeBlock(currentSection.wcs);
          writeBlock(gMotionModal.format(0), zOutput.format(cycle.toolCallPositionWorkCS.z));
        }
        retracted = false;
  ...
  if (getProperty("cleanSubSpindle") && (cycleType == "secondary-spindle-grab")) {
    cleansub()
  }
    writeln("");
    ...
    case "secondary-spindle-grab":
      partOffZPosition = getPartOffZPosition();
     ...
     // writeBlock(mFormat.format(getCode("INTERLOCK_BYPASS", getSecondarySpindle())), formatComment("INTERLOCK BYPASS"));
      ...
      if (cycle.stopSpindle) { // no spindle rotation
        ...
        writeBlock(
          ...
          formatComment((transferType == TRANSFER_PHASE ? "PHASE" : "SPEED") + " SYNCHRONIZATION")
        );
      ...
      writeBlock(mInterferModal.format(getCode("INTERFERENCE_CHECK_OFF", getSpindle(PART))));
      if (partOffZPosition != undefined) {
        writeBlock(gMotionModal.format(0),gFormat.format(54),zOutput.format(partOffZPosition));
      }
...
function cleansub() {
  // Pre SUb spindle grab or return, ensure sub is clean and empty before continuing with transfer
  onCommand(COMMAND_STOP_SPINDLE);
  writeln (" ");
  writeln(formatSequenceNumber() + formatComment("CLEAN SUB.CHUCK"));
  writeBlock(gFormat.format(40),gFormat.format(80),gFormat.format(99),gFormat.format(54),gFormat.format(18));
  writeBlock(gMotionModal.format(30),  "U" + xFormat.format(0), "W" + zFormat.format(0));
  writeBlock(gFormat.format(28), bOutput.format(0), "V" +yOutput.format(0));
 ...
 writeBlock(gMotionModal.format(0), gFormat.format(28), "H" + abcFormat.format(0));
  writeBlock(gFormat.format(50), cOutput.format(0));
  writeBlock(mFormat.format(131));
  writeBlock(mFormat.format(169));
  writeBlock(mFormat.format(0));
  writeComment("TAKE FINISHED PART OUT OF SUB");
  writeComment("CLEAN SUB & MAIN BORE");
  writeComment("CLOSE DOOR & CYCLE START");
}
...
function startSpindle(tappingMode, forceRPMMode, initialPosition) {
  ...
  if (tappingMode) {
    ...
    var code = getCode(method == TRANSFER_PHASE ? "SPINDLE_SYNCHRONIZATION_PHASE" : "SPINDLE_SYNCHRONIZATION_SPEED", getSpindle(PART));
...
function getPartOffZPosition() {
  var numberOfSections = getNumberOfSections();
  for (var i = getCurrentSectionId() + 1; i < numberOfSections; ++i) {
    var section = getSection(i);
    var strategy = section.getParameter("operation-strategy", "");
    if (strategy == "turningPart") {
      return section.getInitialPosition().z;
    } else if (strategy == "turningSecondarySpindleReturn" || strategy == "turningSecondarySpindlePull" || strategy == "turningToolCall") {
      break;
    }
  }
  return undefined;
...
function onSectionEnd() {
  ...
  var isPartHandlingCycle = ["turningSecondarySpindleReturn", "turningSecondarySpindlePull", "turningToolCall"].includes(currentSection.getParameter("operation-strategy"));
  if ((tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) &&
   !machineState.spindlesAreAttached && !machineState.spindlesAreSynchronized && !isPartHandlingCycle) {
  ...
  if (getProperty("cutoffConfirmation") != "false" && partCutoff) {
    writeBlock(gFormat.format(getProperty("cutoffConfirmation") == "true" ? 350 : 133), "B1",formatComment("CONFIRM CUTOFF"));
    onDwell(0.5);
  }
  if (partCutoff) {
