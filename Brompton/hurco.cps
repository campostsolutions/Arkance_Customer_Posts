...
function onOpen() {
  ...
  if (!is3D() && (useVectorOutput && machineConfiguration.isMultiAxisConfiguration())) {
    ...
    var msg = "The postprocessor has been modified to use vector output even though a machine configuration has been defined." + EOL +
      "Be aware that the machine simulation will not be accurate." + EOL +
      "When using vector output, we cannot guarantee that the CNC will not need to rewind during cutting.";
    onComment(msg);
    warning(msg);
  }
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  if (currentSection.isMultiAxis() && useVectorOutput) {
...
var WARNING_ASR = 0;
function writeASR() {
  warningOnce("Caution: No machine definition is specified in either CAM or the postprocessor. As a result, Hurco Automatic Safe Repositioning (ASR) G8.2 is used." + EOL +
    "CAM simulation cannot accurately reflect movements generated by the ASR feature.", WARNING_ASR
  );
  var position = getFramePosition(currentSection.getInitialPosition());
  var direction = currentSection.getInitialToolAxis();
  forceXYZ();
  forceABC();
  writeBlock(
    gFormat.format(8.2),
...
function setWorkPlane(abc) {
  ...
  writeStartBlocks(workplaneIsRequired, function () {
    ...
    if (settings.workPlaneMethod.useTiltedWorkplane) {
      ...
      if (machineConfiguration.getNumberOfAxes() == 5 && machineConfiguration.getAxisU().getCoordinate() > machineConfiguration.getAxisV().getCoordinate()) {
        workPlaneCode = 68.3;
      }
      writeBlock(
        ...
        useVectorOutput ?
          ...
          formatWords(
            conditional(machineConfiguration.isMachineCoordinate(0), "A" + abcFormat.format(machineABC.x % (Math.PI * 2))),
            conditional(machineConfiguration.isMachineCoordinate(1), "B" + abcFormat.format(machineABC.y % (Math.PI * 2))),
            conditional(machineConfiguration.isMachineCoordinate(2), "C" + abcFormat.format(machineABC.z % (Math.PI * 2))))
      ); // set frame
      if (useVectorOutput) {
        writeASR();
      }
